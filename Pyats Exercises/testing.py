...
class ShowVrf:
    ...
    def vrf_cli(self, testbed, vrf='', output=None):
...
class TriggerShowVrf(aetest.Testcase):
    @aetest.test
    def show_vrf(self, testbed):
        vrf_obj = ShowVrf()
        output = vrf_obj.vrf_cli(testbed)
        print(output)
...
class ShowVlan:
    ...
    def vlan_cli(self, testbed, vlan='', output=None):
        device = testbed.devices["Whatever"]
        if output is None:
            if vlan:
        ...
        for line in out.splitlines():
            ...
            if m:
                ...
                if vlan_id not in vlan_dict['vlan']:
...
class TriggerShowVlan(aetest.Testcase):
    @aetest.test
